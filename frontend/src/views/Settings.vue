            ...
            <div class="page-header-compact">
                <div class="header-left">
                    ...
                    <div class="header-tabs">
                        ...
                        <button class="tab-btn" :class="{ active: activeTab === 'tenants' }"
                        ...
                        <button class="tab-btn" :class="{ active: activeTab === 'ai' }"
                    ...
                    <div class="glass-card" style="max-width: 600px; margin: 0 auto; padding: 2rem;">
                        <div style="text-align: center; margin-bottom: 2rem;">
                            ...
                            <h2 style="font-size: 1.5rem; font-weight: 600; color: #111827; margin-bottom: 0.5rem;">
                                Privacy & Anonymity</h2>
                            <p style="color: #6b7280;">Adjust how sensitive financial data is displayed across the
                                application.</p>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group-flex" style="display: flex; align-items: center; gap: 1rem;">
                                <input type="number" v-model.number="settingsStore.maskingFactor" min="1"
                                    class="form-input" placeholder="1" style="flex: 1;" />
                        ...
                        <div class="info-box"
                            style="background: #f3f4f6; padding: 1rem; border-radius: 0.5rem; margin-top: 1.5rem; display: flex; align-items: start; gap: 0.75rem;">
                ...
                <div v-if="activeTab === 'accounts'" class="tab-content animate-in">
                    ...
                    <div class="account-control-bar mt-8 mb-6">
                        ...
                        <div class="search-bar-premium no-margin" style="flex: 1; max-width: 300px;">
                            ...
                            <input type="text" v-model="searchQuery" placeholder="Search accounts..."
                                class="search-input">
                        </div>
                ...
                <div v-if="activeTab === 'emails'" class="tab-content animate-in">
                    ...
                    <div class="account-control-bar mb-6">
                        <div class="search-bar-premium no-margin" style="flex: 1; max-width: 300px;">
                            ...
                            <input type="text" v-model="searchQuery" placeholder="Search email accounts..."
                                class="search-input">
                        </div>
                        <div class="header-with-badge"
                            style="margin-left: auto; display: flex; align-items: center; gap: 0.75rem;">
                    ...
                    <div v-if="syncStatus && syncStatus.status !== 'running'"
                        :class="['sync-alert-premium', syncStatus.status]">
                                    ...
                                    <form @submit.prevent="saveAiSettings" class="space-y-6">
                                        <div class="form-row">
                                            <div class="ai-input-group half">
                                                ...
                                                <CustomSelect v-model="aiForm.provider"
                                                    :options="[{ label: 'Google Gemini', value: 'gemini' }]" />
                                            </div>
                                            <div class="ai-input-group half">
                                                <label class="ai-input-label">Model Selection</label>
                                                <div class="flex gap-2">
                                                    <CustomSelect v-model="aiForm.model_name" :options="aiModels"
                                                        class="flex-1" />
                                                    <button type="button" @click="fetchAiModels" class="btn-icon-circle"
                                        ...
                                        <div class="ai-input-group">
                                            <div class="flex justify-between items-center mb-2">
                                                ...
                                                <button type="button" class="text-xs text-indigo-600 font-bold"
                                                    @click="aiForm.prompts.parsing = 'Extract transaction details...'">Reset
                                                    to Default</button>
                                            </div>
                                            <textarea v-model="aiForm.prompts.parsing"
                                                class="form-input font-mono text-xs" rows="4"></textarea>
                        ...
                        <div class="ai-playground">
                            <div class="ai-card">
                                ...
                                <div class="ai-card-body">
                                    ...
                                    <textarea v-model="aiTestMessage"
                                        class="form-input text-xs mb-4 bg-gray-50 border-dashed" rows="3"
                                        placeholder="e.g. Spent Rs 500 at Amazon..."></textarea>

                                    <button @click="testAi" class="btn-verify width-full mb-6" :disabled="aiTesting">
...
.global-sync-alert.completed {
    background: #ecfdf5;
    color: #059669;
}

.global-sync-alert.error {
    background: #fef2f2;
    color: #dc2626;
}
...
/* Card Specifics */
.glass-card.email-config,
.glass-card.tenant-card {
    min-height: 100px;
    justify-content: space-between;
}
...
.member-profile-card {
    position: relative;
    padding: 0 !important;
    /* Reset standard padding to allow custom layout */
    overflow: hidden;
    display: flex;
    flex-direction: column;
    align-items: center;
...
.gradient-avatar.ring-gold {
    border-color: #fbbf24;
    background: linear-gradient(135deg, #fffbeb, #fff);
}

.gradient-avatar.ring-blue {
    border-color: #818cf8;
    background: linear-gradient(135deg, #eef2ff, #fff);
}

.gradient-avatar.ring-green {
    border-color: #34d399;
    background: linear-gradient(135deg, #ecfdf5, #fff);
}

.gradient-avatar.ring-gray {
    border-color: #9ca3af;
}
...
.profile-stats {
    ...
    display: flex;
    justify-content: center;
    /* Center since only 1 stat for now */
    gap: 1rem;
}
...
.modal-section:nth-child(2) .form-field:nth-child(1) {
